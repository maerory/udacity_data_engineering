...
def process_song_file(cur, filepath):
    """ 
        Reads in the json data from 'song_data'
        execute the insert quaries to insert the corresponding data
        
        Arguments:
            cur {object}: Cursor for Postgre SQL
            filepath {list}: path to the json data file
        Returns:
            None
    """
    # open song file
...
def process_log_file(cur, filepath):
    """ 
        Reads in the json data from 'log_data'
        execute the insert quaries to insert the corresponding data
...
def process_data(cur, conn, filepath, func):
    """ 
        collect all the json files in the directory
        and put through the corresponding pipeline function defined above
        
        Arguments:
            cur {object}: Cursor for Postgre SQL
            conn {object}: psycopg2 connection to the DB
            filepath {list}: path to the json data file
            func {object}: etl pipeline function
        Returns:
            None
    """
